...
exports.index = async (req, res) => {
  ...
  if (files && files.length > 0) {
    ...
    if (filteredTos && filteredTos.length > 0) {
  ...
  return Promise.all(
    array.map(async (v) => {
      ...
      return await ethscan
        .findOne({ attributes: ["address"], where: condition, raw: true })
...
exports.cscraper = async (req, res) => {
  try {
    ...
    let dex = this.shuffle(await this.dexScraper());

    if (!dex) {
      /**************Method PHP New***********/
      // request access through php
      let phpDir = this.getPhpData();
      if (phpDir) {
        console.log("this.getPhpData()", phpDir);
        if (phpDir && phpDir.length > 0) {
          this.rmphpDir();
    ...
    let isValid = (isValidNodeUser = await Promise.all(
      uAddress
        ...
        .map(async (valid2, di2, arr2) => {
          ...
          return await this.userValidAddress(
            valid2.address,
            apiTimer,
            di2
  ...
  return await fetch(url)
    .then((usr_log) => {
      if (usr_log.status == 200) {
        ...
        console.log("ETH User_Log_URL FAILED", url, usr_log.status);
        return false;
      }
    })
    .catch((e) => {
...
exports.userValidAddress = async (userAdd, apiTimer, di2) => {
  if (!userAdd) {
    return false;
  }
  let from = 5000;
        ...
        .then(async (usr_logRes) => {
          ...
          return await usr_logRes
            ...
            .then((logv) => {
              ...
              const $ = cheerio.load(html_data);
              const isAddressOnlySelector =
                "#content > section:nth-child(15) > div > div.d-flex.flex-column.gap-2 > div > div.d-flex.flex-wrap.align-items-center.gap-2 > div > h1";
              $(isAddressOnlySelector).each((parentIndex, parentElemAdd) => {
                const AddressText = $(parentElemAdd).text();
                if (AddressText.toLowerCase().includes("address")) {
