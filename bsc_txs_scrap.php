...
function contractScraper($dex_arr)
{
    ...
    foreach ($dex_arr as $dexkey => $dexvalue) {
        ...
        $get_contract_url = "https://bscscan.com/tx/" . $dexvalue;
...
function transactionLog($contract_arr)
{
    ...
    foreach ($contract_arr as $contractkey => $contractvalue) {
        ...
        $get_tx_url = "https://bscscan.com/txs?a=" . $contractvalue;
        ...
        $nodes = $finder->query('//*[@id="ContentPlaceHolder1_divTransactions"]/div[2]/table/tbody/tr/td[10]/div/a[1]');
...
function userValidAddress($get_all_user_addr_arr)
{
    ...
    foreach ($get_all_user_addr_arr as $addresskey => $addressvalue) {
        ...
        $get_addr_url = "https://bscscan.com/address/" . $addressvalue;
        ...
        foreach ($nodes as $node) {
            ...
            echo $get_addr_url ."\t". $result['http_code']. "\t" . $res ."\t \n";
...
function index()
{
    ...
    $finder = new DomXPath($dom);
    // $nodes = $finder->query("//*[contains(concat(' ', normalize-space(@class), ' '), ' $classname ')]");
    $nodes = $finder->query('//*[@id="ContentPlaceHolder1_divTransactions"]/div[2]/table/tbody/tr/td[8]/div/a[2]');
    $tmp_dom = new DOMDocument();
    foreach ($nodes as $node) {
...
function file_line_by_line($filename)
{
    ...
    // $path    = __DIR__ . '/php/etherscan/bsc/txn/' . $filename;
    $path    = __DIR__ . '/bsc/txn/' . $filename;
...
{
    // $path    = __DIR__ . '/php/etherscan/bsc/txn/';
    $path    = __DIR__ . '/bsc/txn/';
...
if ($new_file_record) {
    ...
    echo $path = __DIR__ . '/bsc/txn/' . $title;
    $fpc = file_put_contents($path, (json_encode($new_file_record, JSON_PRETTY_PRINT))); //generate json file 
    // header('Content-Type: application/json');
    // header('Content-Disposition: attachment; filename=data.json');
    // header('Expires: 0'); //No caching allowed
    // header('Cache-Control: must-revalidate');
    // header('Content-Length: ' . strlen((json_encode($new_file_record, JSON_PRETTY_PRINT))));
    // file_put_contents('php://output', (json_encode($new_file_record, JSON_PRETTY_PRINT)));
    var_dump($fpc);
    $time = 1;
    $title = 'block_txs_scrap_a_' . $time . '.json';
    $myfile = fopen($path, "w"); //or die("Unable to open file!");                
    fwrite($myfile, json_encode($new_file_record, JSON_PRETTY_PRINT));
    fclose($myfile);
    echo 'File Created 1'.PHP_EOL;    
}
